...
{
    public abstract class Gml2SosiService
    {
        private readonly IHodeMapper _hodeMapper;
        private readonly ILogger<Gml2SosiService> _logger;

        protected Gml2SosiService(
            IHodeMapper hodeMapper,
            ILogger<Gml2SosiService> logger)
        {
            _hodeMapper = hodeMapper;
            _logger = logger;
        }

        protected async Task<List<SosiElement>> CreateSosiDocumentAsync(
